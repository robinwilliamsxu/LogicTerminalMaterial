 ...
 * @author mstamper
 */
public class PropagateTerminalMaterial implements IXLogicAction {
		...
		for (IXWire wire : wireList) {
			...
			for (IXAbstractPin pin : pins) 
				...
				if (pin instanceof IXCavity) {
					...
					if (devPin != null) {
						String contactMaterial = devPin
								.getAttribute("ContactMaterial");
						if (contactMaterial != null) {
							if (contactMaterial != null && contactMaterial != "" ) {
								if (i == 0)
    ...
    public String getDescription() {
        return "[ Mentor ]Terminal Material from DevicePin";
    }
    ...
    public String getName() {
        return "[ Mentor ]TerminalMaterialFromDevicePin";
    }
